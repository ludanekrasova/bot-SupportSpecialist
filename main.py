...
def get_text_messages(message):
    if message.text == "/start":
        ...
        if similarity >= 0.7:
            if dev == True:
                bot.send_message(message.from_user.id, f"{answer} \n\n Класс ответа: {answer_class}   {category} \n\n Схожесть: {similarity}", parse_mode="Markdown")
            else:
                bot.send_message(message.from_user.id, f"{answer} ", parse_mode="Markdown")

        elif similarity <= 0.2:
            bot.send_message(message.from_user.id, f"К сожалению, я не могу ответить на ваш вопрос. Пожалуйста, дождитесь ответа оператора.", parse_mode="Markdown")
    ...
    def callback_handler(call):
        ...
        if callback == "correct":
            if dev == True:
                ...
                bot.send_message(call.message.chat.id, f"{answer} ", parse_mode="Markdown")
        elif callback == "incorrect":
            bot.send_message(call.message.chat.id, f"Пожалуйста, сформулируйте вопрос более развернуто.", parse_mode="Markdown")
